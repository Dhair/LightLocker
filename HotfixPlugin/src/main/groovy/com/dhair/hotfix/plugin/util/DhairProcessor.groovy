package com.dhair.hotfix.plugin.util
...
class DhairProcessor {
        ...
        if (jarFile) {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                if (shouldProcessClassInJar(entryName, includePackage, excludeClass)) {
                    ...
                    hashFile.append(DhairMapUtils.format(entryName, hash))

                    if (DhairMapUtils.notSame(map, entryName, hash)) {
                        DhairFileUtils.copyBytesToFile(bytes, DhairFileUtils.touchFile(patchDir, entryName))
                    }
                } else {
                ...
                mv = new MethodVisitor(Opcodes.ASM4, mv) {
                    ...
                    void visitInsn(int opcode) {
                        if ("<init>".equals(name) && opcode == Opcodes.RETURN) {
                            super.visitLdcInsn(Type.getType("Lcn/dhair/hotfix/Hack;"));
                        }
    ...
    static boolean shouldProcessClassInJar(String entryName, HashSet<String> includePackage, HashSet<String> excludeClass) {
        return entryName.endsWith(".class") && !entryName.startsWith("cn/dhair/hotfix") && DhairSetUtils.isIncluded(entryName, includePackage) && !DhairSetUtils.isExcluded(entryName, excludeClass) && !entryName.contains("android/support/")
    }
